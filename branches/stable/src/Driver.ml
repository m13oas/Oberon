...
let _ =
  ...
  let options = 
    ...
    (fun _ -> [])
  in
  let languages = [|L1.top; L2.top; L3.top; L4.top|] in
  let _ :: args = Array.to_list Sys.argv in  
  let perFile l conf file =
    let source, errors, pp, c = 
      if Filename.check_suffix file extension
      then 
        let basename = Filename.chop_suffix file extension in 
        file, basename ^ ".errors", basename ^ "_pp" ^ extension, basename ^ ".c"
      else 
        file ^ extension, file ^ ".errors", file ^ "_pp" ^ extension, file ^ ".c"
    in
    ...
    (match conf.get "c" with 
     | Some Flag -> 
        verify (fun _ -> verify (output c) (l#generate ())) (l#typecheck ())
     | _ -> ()
    );
