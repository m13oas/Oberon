...
let print typ decl stmt =
  ...
  in  
  let formalParameters args = rboxed (hvboxed (mseq parameter args)) in
...
let print_proto_c typ =
  let declaration p = vert [seq [print_header_c typ p; string ";"]] in
  function [] -> [] | t -> [vert (List.map declaration t)]
...
let resolve restricted env typ decl stmt pdecls =
  ...
  let inner ((name, args, decls, stmts) as t) =
    ...
    if restricted then ignore (env#update name (`Proc (iname, args)));
    let stmts = list (List.map stmt stmts) in
    env#up (); 
    (if restricted then env#update name (`Proc (iname, args)) else !! ()) -?->>
    return (
      tuple (list [msg'; msg''], tuple (stmts, decls)) -?-> 
      (fun (_, (stmts, decls)) -> (iname, args', decls, stmts))
    )
  in
  let update ((name, args, decls, stmts) as t) =
    let iname = env#getInternal name in
    env#update name (`Proc (iname, args)) -?-> return t
  in
  let checkProc f (name, args, d, s) =
    resolveArgs args -?->> 
    (fun args -> 
       f (name, args, d, s) -?-> (fun x -> name, x)
    )
  in
  if restricted 
  then
    ...
    let m, pdecls = Common.resolveDecls (checkProc update) pdecls in
